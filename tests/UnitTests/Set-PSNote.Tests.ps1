...
Describe "Set-PSNote" {
    ...
    Context "Updating existing notes" {
        ...
        BeforeEach {
            ...
            New-PSNote -Name 'SetTestNote' -Snippet 'Get-Process' -Details 'Original details' -Tags 'Test' -Catalog 'Personal'
        }

        It "updates Snippet of an existing note" {
            Set-PSNote -Name 'SetTestNote' -Snippet 'Get-Service' -Catalog 'Personal'
            
            $result = Get-PSNote -Name 'SetTestNote'
            $result.Snippet | Should -Be 'Get-Service'
        }

        It "updates Details of an existing note" {
            Set-PSNote -Name 'SetTestNote' -Details 'Updated details' -Catalog 'Personal'
        ...
        It "updates Tags of an existing note" {
            Set-PSNote -Name 'SetTestNote' -Tags 'Updated', 'NewTag' -Catalog 'Personal'
        ...
        It "updates Alias of an existing note" {
            Set-PSNote -Name 'SetTestNote' -Alias 'new-alias' -Catalog 'Personal'
        ...
        It "updates with ScriptBlock parameter" {
            ...
            Set-PSNote -Name 'SetTestNote' -ScriptBlock $scriptBlock -Catalog 'Personal'
        ...
        It "updates with ScriptPath parameter" {
            ...
            Set-PSNote -Name 'SetTestNote' -ScriptPath $scriptFile -Catalog 'Personal'
        ...
        It "updates multiple properties at once" {
            Set-PSNote -Name 'SetTestNote' -Snippet 'Get-Date' -Details 'New details' -Tags 'Updated','Time' -Alias 'date-alias' -Catalog 'Personal'
    ...
    Context "Creating notes when they don't exist" {
        ...
        It "creates a new note with warning when note doesn't exist" {
            Set-PSNote -Name 'NonExistentNote' -Snippet 'Get-Date' -Details 'New note' -Catalog 'Personal' -WarningVariable warn -WarningAction SilentlyContinue
            
            $result = Get-PSNote -Name 'NonExistentNote'
        ...
        It "creates a new note with blank alias when creating non-existent note" {
            Set-PSNote -Name 'CreatedNoteBlank' -Snippet 'Test' -Catalog 'Personal'
            
            $result = Get-PSNote -Name 'CreatedNoteBlank'
            $result.Alias | Should -Be ''
        }

        It "creates a new note with alias when creating non-existent note" {
            Set-PSNote -Name 'CreatedNoteAlias' -Snippet 'Test' -Catalog 'Personal' -Alias 'CreatedNoteAlias'
            
            $result = Get-PSNote -Name 'CreatedNoteAlias'
            $result.Alias | Should -Be 'CreatedNoteAlias'
        }

        It "creates a new note with custom alias when creating non-existent note" {
            Set-PSNote -Name 'CreatedWithAlias' -Snippet 'Test' -Alias 'custom-alias' -Catalog 'Personal'
            
            $result = Get-PSNote -Name 'CreatedWithAlias'
        ...
        It "creates a new note from ScriptPath when note doesn't exist" {
            ...
            Set-PSNote -Name 'CreatedFromScriptPath' -ScriptPath $scriptFile -Catalog 'Personal'

            $result = Get-PSNote -Name 'CreatedFromScriptPath'
            $result.Snippet | Should -Be $scriptFile
            $result.Kind | Should -Be 'Script'
        }
    }
    ...
    Context "Pipeline input" {
        It "accepts Note from pipeline by property name" {
            New-PSNote -Name 'PipelineTestNote' -Snippet 'Get-Process' -Catalog 'Personal' -Force
            $noteObject = [PSCustomObject]@{
                Name    = 'PipelineTestNote'
                Snippet = 'Get-Service'
                Catalog = 'Personal'
            }
            ...
            $result = Get-PSNote -Name 'PipelineTestNote'
            $result.Snippet | Should -Be 'Get-Service'
        }
        ...
        It "accepts ScriptPath from pipeline by property name" {
            ...
            $result = Get-PSNote -Name 'PipelineScriptPathNote'
            $result.Snippet | Should -Be $scriptFile
            $result.Kind | Should -Be 'Script'
        }
    }
    ...
    Context "Alias validation" {
        ...
        It "accepts valid alias with letters, numbers, dashes, and underscores" {
            { Set-PSNote -Name 'ValidAliasSet' -Alias 'valid-alias_123' -Snippet 'Test' -Catalog 'Personal' } | Should -Not -Throw
        }

        It "throws an error for alias with spaces" {
            { Set-PSNote -Name 'InvalidAliasSet1' -Alias 'invalid alias' -Snippet 'Test' -Catalog 'Personal' } | Should -Throw
        }

        It "throws an error for alias with special characters" {
            { Set-PSNote -Name 'InvalidAliasSet2' -Alias 'invalid@alias' -Snippet 'Test' -Catalog 'Personal' } | Should -Throw
        }
    }
    ...
    Context "Parameter sets" {
        ...
        It "accepts Snippet parameter" {
            { Set-PSNote -Name 'SnippetParamSet' -Snippet 'Get-Date' -Catalog 'Personal' } | Should -Not -Throw
        }

        It "accepts ScriptBlock parameter" {
            { Set-PSNote -Name 'ScriptBlockParamSet' -ScriptBlock { Get-Date } -Catalog 'Personal' } | Should -Not -Throw
        }

        It "accepts ScriptPath parameter" {
            $scriptFile = Join-Path $script:TestDir 'ScriptPathParamSet.ps1'
            Set-Content -Path $scriptFile -Value 'Get-ChildItem' -Force
            { Set-PSNote -Name 'ScriptPathParamSet' -ScriptPath $scriptFile -Catalog 'Personal' } | Should -Not -Throw
        }

        It "converts ScriptBlock to string for storage" {
            $sb = { Get-Process | Select-Object -First 5 }
            Set-PSNote -Name 'ScriptBlockConversionSet' -ScriptBlock $sb -Catalog 'Personal'
            
            $result = Get-PSNote -Name 'ScriptBlockConversionSet'
        ...
        It "throws when ScriptPath does not exist" {
            ...
            { Set-PSNote -Name 'MissingScriptPath' -ScriptPath $missingFile -Catalog 'Personal' } | Should -Throw
        }
    }
...
$array
'@
            Set-PSNote -Name 'MultilineSetTest' -Snippet $multilineSnippet -Catalog 'Personal'
            
            $result = Get-PSNote -Name 'MultilineSetTest'
    ...
    Context "Global alias creation" {
        ...
        It "creates or updates a global alias for the note" {
            Set-PSNote -Name 'AliasSetCreation' -Snippet 'Get-Date' -Alias 'test-set-alias' -Catalog 'Personal'
