...
Describe 'PSNote Class' {
    ...
    Context 'Kind and Target behavior' {
        It 'defaults Kind to Snippet and Target to Snippet (5-parameter constructor)' {
            $note = [PSNote]::new(
                'MyNote',
                'Write-Host "Hello"',
                'A greeting snippet',
            ...
            $note.Target | Should -Be 'Write-Host "Hello"'
            # Back-compat: Snippet remains populated
            $note.Snippet | Should -Be $note.Target
        }

        It 'supports Script kind via Kind/Target constructor' {
            $note = [PSNote]::new(
                'RunScript',
                [PSNoteKind]::Script,
            ...
            $note.Target | Should -Be '.\Scripts\Do-The-Thing.ps1'
            # Script notes do not require Snippet to be populated
            $note.Snippet | Should -BeNullOrEmpty
        }

        It 'migrates legacy objects by mapping Snippet to Target when Kind/Target are absent' {
            $obj = [pscustomobject]@{
                Note    = 'LegacyNote'
                Snippet = 'Get-Date'
                Details = 'legacy'
            ...
            $note.Kind | Should -Be ([PSNoteKind]::Snippet)
            $note.Target | Should -Be 'Get-Date'
            $note.Snippet | Should -Be 'Get-Date'
        }

        It 'parses Script Kind and Target from object data' {
            $obj = [pscustomobject]@{
                Note    = 'ScriptNote'
                Kind    = 'Script'
                Target  = 'C:\Temp\Do.ps1'
                Details = 'script note'
                Alias   = 'do'
                Tags    = @('script')
                Catalog = 'TestCatalog'
            ...
            $note.Kind | Should -Be ([PSNoteKind]::Script)
            $note.Target | Should -Be 'C:\Temp\Do.ps1'
        ...
        It 'falls back to Snippet as Target when Target is missing but Snippet exists' {
            $obj = [pscustomobject]@{
                Note    = 'TargetMissing'
                Kind    = 'Script'
                Snippet = '.\relative.ps1'
            ...
            $note.Kind | Should -Be ([PSNoteKind]::Script)
            $note.Target | Should -Be '.\relative.ps1'
        }
